...
def list_issues(
    ...
    if outliers_only:
        ...
        for outlier in OutlierEngine(db, tenant_id=tenant_id).list_outliers():
            if vessel and vessel.lower() not in f"{outlier['vcn'] or ''} {outlier['vessel_name'] or ''}".lower():
                continue
            results.append(QualityIssueSchema(
                id=outlier["id"], rule_id=outlier["rule_id"] or "OUTLIER", vessel_call_id=outlier["vessel_call_id"],
                ...
                scope=outlier["outlier_type"], vcn=outlier["vcn"],
                vessel_name=outlier["vessel_name"], disposition="EXCLUDED" if outlier["is_excluded_from_kpi"] else "REVIEW",
                workflow_state="EXCLUDED" if outlier["is_excluded_from_kpi"] else "OPEN", created_at=outlier["detected_at"],
                issue_class="OUTLIER",
                original_values={
                    "observed_value": outlier["observed_value"],
                    "threshold": outlier["benchmark_or_p90"],
                    "threshold_label": outlier["threshold_label"],
                    "source_record_ids": outlier["source_record_ids"],
                    "evidence": outlier["evidence"] or {},
                    "legacy_record": outlier["legacy_record"],
                },
