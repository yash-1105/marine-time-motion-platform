...
def ask(
    ...
    principal: UserPrincipal = Depends(require("view", "copilot")),
):
    try:
        response, msg = CopilotService(db, principal).ask(body.question, body.conversation_id)
    ...
    log_audit_event(
        ...
        details={
            "scope": principal.data_scope.model_dump(),
...
def conversations(db: Session = Depends(get_db), principal: UserPrincipal = Depends(require("view", "copilot"))):
    rows = db.execute(
        ...
        .where(CopilotConversation.tenant_id == tenant(principal), CopilotConversation.owner_id == principal.user_id)
        .order_by(CopilotConversation.updated_at.desc())
    ).scalars()
    return [{"conversation_id": x.conversation_id, "title": x.title, "created_at": x.created_at} for x in rows]
...
def feedback(
    ...
    msg = db.execute(
        ...
        .where(
            ...
            CopilotConversation.owner_id == principal.user_id,
            CopilotConversation.tenant_id == tenant(principal),
        )
    ).scalar_one_or_none()
    if not msg:
        raise HTTPException(404, "Message not found")
    f = CopilotFeedback(message_id=msg.id, rating=body.rating, comment=body.comment)
    db.add(f)
    db.commit()
    log_audit_event(
        db,
        ...
        principal.user_id,
        principal.email,
        details={"message_id": message_id, "rating": body.rating},
    )
