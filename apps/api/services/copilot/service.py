...
class GovernedTools:
 ...
 def dashboard(self, args: dict[str, Any]): return ExecutiveDashboardService(self.db,self.tenant).get_executive_summary(vessel_type=args.get("vessel_type"),cargo_type=args.get("cargo_type"))
 def lead(self,args):
 ...
 def delays(self,args):
  ...
  data=DelayService(self.db,self.tenant).get_delays_summary(); return {"status":"AVAILABLE","summary":data,"evidence":["/delays"]}
 def stats(self,args):
  v=self.lead(args)
 ...
 def journey(self,args):
  call_id=args.get("vessel_call_id"); vc=self.db.execute(select(VesselCall).where(VesselCall.id==call_id,VesselCall.tenant_id==self.tenant)).scalar_one_or_none()
 ...
 def correlation(self,args):
  ...
  data=[]
  for d in defs:
   rows=self.db.execute(select(LeadTimeResult.vessel_call_id,LeadTimeResult.duration_hours).join(VesselCall).where(LeadTimeResult.definition_id==d.id,VesselCall.tenant_id==self.tenant,VesselCall.is_merged==False,LeadTimeResult.status=="AVAILABLE")).all(); data.append(dict(rows))
...
class CopilotService:
 ...
 def sarvam_choice(self, question: str):
  ...
  if not settings.sarvam_api_key: return self.choose(question)
  tools=[{"type":"function","function":{"name":n,"description":"Call governed port analytics only","parameters":{"type":"object","properties":{},"additionalProperties":False}}} for n in TOOL_NAMES]
  ...
  try:
   raw=json.loads(urllib.request.urlopen(req,timeout=20).read()); call=raw["choices"][0]["message"].get("tool_calls",[])[0]; return call["function"]["name"],json.loads(call["function"].get("arguments") or "{}")
  except Exception: return self.choose(question)
 def ask(self,question,conversation_id=None):
  cid=conversation_id or f"cop-{uuid4().hex}"; convo=self.db.execute(select(CopilotConversation).where(CopilotConversation.conversation_id==cid, CopilotConversation.owner_id==self.p.user_id)).scalar_one_or_none()
  ...
  tool,args=self.sarvam_choice(question); result=GovernedTools(self.db,self.p).execute(tool,args)
