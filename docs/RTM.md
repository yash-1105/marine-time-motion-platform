...
| UX-001 | Cross-cutting visual presentation | A cohesive maritime enterprise design system is applied to every existing frontend route without changing routes, data contracts, calculations, actions, or state transitions. | `apps/web/src/app/globals.css`, shared UI components, application shell | frontend lint, type check, production build, route smoke checks | IMPLEMENTED |
| UX-002 | §20 accessibility and operational dashboards | Navigation, controls, tables, status indicators, focus states, and reduced-motion behavior remain accessible and readable at desktop operational densities. | `apps/web/src/components`, route presentation markup | frontend lint, production build, desktop route inspection | IMPLEMENTED |
| UX-003 | Cross-cutting page hierarchy | Each sidebar route exposes one compact primary heading in the application header; redundant body-level titles and introductory subtitles are removed while page metadata and actions remain available in a compact toolbar. | `apps/web/src/components/AppShell.tsx`, `apps/web/src/components/ui/PageHeader.tsx`, sidebar route presentation markup | frontend lint, type check, production build, sidebar route inspection | IMPLEMENTED |
| UX-004 | §12 operational dashboards | The Executive Dashboard preserves its governed values, drill-downs, and information architecture while improving metric hierarchy, comparison density, action affordance, and responsive presentation. | `apps/web/src/app/page.tsx` | frontend lint, type check, production build, responsive dashboard inspection | IMPLEMENTED |
| DQ-EXT-001 | FRD §§4.2–4.5 | Validate source completeness, invalid timestamp values, exact duplicates, configured DAG chronology, service request/schedule/served inputs, and schedule-before-request deterministically before candidate activation. | `config/quality_rules.yaml`, `apps/api/services/quality/engine.py` | `tests/test_dq_engine.py`, `make validate` | IMPLEMENTED |
| DQ-EXT-002 | FRD DQ review workflow | Expose source file/sheet/row/original values and filters for severity, rule, vessel, file, service, movement and distinct outlier review class. | `apps/api/routers/quality.py`, `apps/web/src/app/data-quality/page.tsx` | frontend lint/typecheck/build | IMPLEMENTED |
| DQ-EXT-003 | FRD DQ exclusion workflow | Allow an authorised administrator to logically exclude selected staging rows with an audit trail, retaining raw evidence and rebuilding canonical/analytics/KPI/dashboard outputs. | `apps/api/routers/quality.py`, `apps/api/services/ingestion/pipeline.py` | `tests/test_dq_engine.py`, `make validate` | IMPLEMENTED |
| ING-MULTI-002 | FRD ingestion / governed dataset groups | Discover governed worksheets per file, retain successfully parsed supplemental workbooks as explicitly skipped evidence, and require at least one governed worksheet plus the existing group-level VesselCalls requirement. Corrupt workbooks and malformed governed worksheets remain fatal. | `apps/api/services/ingestion/pipeline.py`, `apps/api/routers/ingestion.py`, `apps/web/src/app/ingestion/page.tsx` | `test_mixed_group_skips_non_governed_workbook`, `test_non_governed_only_group_fails`, `test_fatal_file_schema_failure_keeps_group_inactive`, isolated master/split equivalence acceptance | IMPLEMENTED |
| DELAY-UI-001 | FRD §§4.3–4.4 delay analysis | Expose full-dataset Pilotage, Berthing, Towage Wait, Tug/Towage and Anchorage delay summaries from governed backend timestamps; expose Pilotage and Berthing duration min/max from their explicit governed event pairs while keeping Towage unavailable without its required end event. Table filters remain scoped to the service-timing and detailed-delay tables. | `apps/api/services/delays/service.py`, `apps/web/src/app/delays/page.tsx` | `test_service_delay_overview_and_duration_ranges_are_governed_and_unfiltered`, frontend lint/typecheck/build | IMPLEMENTED |
| JOURNEY-UI-001 | FRD §§4.3, 4.4.3–4.4.5 | Show requested, scheduled/assigned and actual/served timestamps with Scheduling Gap and Execution Delay as distinct concepts in Vessel Journey service detail. | `apps/api/routers/journey.py`, `apps/web/src/app/vessel-journey/page.tsx` | `test_service_timing_preserves_frd_delay_signs_and_legs`, frontend lint/typecheck/build | IMPLEMENTED |
| FRD-V2-421 | FRD v2 §4.2.1 | Register every revised vessel/cargo, marine-event and berth attribute with its datatype, canonical key, source aliases and current source-availability state; preserve additional governed attributes through the extensible canonical model. | `config/frd_v2_data_dictionary.yaml`, `config/events.yaml`, `config/aliases.yaml`, `apps/api/services/ingestion/standardization.py`, canonical models | `tests/test_frd_v2_alignment.py::test_v2_data_dictionary_is_complete_and_mappable`, ingestion mapping tests | IMPLEMENTED |
| FRD-V2-443 | FRD v2 §4.4.3 | Keep submission/requested/scheduled/actual service timestamps independent; calculate planning lead time, scheduling gap, signed execution delay and Arrival/Sailing/Shifting delay frequency with exact v2 bands. | `apps/api/services/analytics/duration_semantics.py`, `apps/api/services/delays/service.py`, `apps/web/src/app/delays/page.tsx` | focused delay-frequency boundary and service-timing tests | IMPLEMENTED |
| FRD-V2-446 | FRD v2 §4.4.6 | Detect supported exception scenarios deterministically using governed P90/P95/P30/mean+3σ thresholds, classify them into the five v2 categories, and register unavailable scenarios with exact missing inputs. | `config/outlier_rules.yaml`, `apps/api/services/outliers/engine.py`, `apps/api/routers/outliers.py`, `apps/web/src/app/delays/page.tsx` | focused outlier registry, threshold and tenant-scope tests | IMPLEMENTED |
