...
FRD-v2 revision `q5r6s7t8u9v0` is also additive. It adds canonical extension envelopes and tenant-indexed structured outlier evidence, backfilling the outlier tenant from each linked vessel call. It does not rewrite raw/staging evidence or active vessel-call data. Apply the migration before starting the v2 API or worker.
